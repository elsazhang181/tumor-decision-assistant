...
export default function Home() {
  ...
  const sendMessage = async (content: string) => {
    ...
    // 各环节关键词及权重（key 必须与 scores 中的 key 一致）
    const keywordScores: Record<string, { keywords: string[]; weight: number }[]> = {
      ...
      'treatment-plan': [
        { keywords: ['化疗', '放疗', '手术', '靶向', '免疫治疗', '免疫', '耐药', '疗程', '方案', '治疗效果', '副作用', '不良反应', '效果', '起效', '没效果', '降低', '上升', '内镜', 'esd', 'emr', '切除', '抑酸', '胃镜', '肠镜', '结肠镜', '术前', '术后', '恢复', '息肉', '溃疡', '肿块', '良性', '恶性', '肿瘤', '癌症', '癌', 'cea', 'ca199', 'ca724', 'ca242', '卡培', '卡培他滨', '奥沙利铂', '氟尿嘧啶', '紫杉醇', '顺铂', '新辅助', '辅助化疗', '姑息', '根治', 'mdt', '多学科', 't3', 't2', 't1', 'n0', 'n1', 'm0', 'm1', 'tnm', '结肠癌', '直肠癌', '肠癌', '胃癌', '肺癌', '乳腺癌', '肝癌', '胰腺癌'], weight: 2 },
        { keywords: ['治疗', '吃药', '用药', '药物', '处方', '剂量', '用法', '疗程', '周期', '几个疗程', '第几疗程', '复查', '随访', 'ct', '核磁', 'mri', 'pet-ct', '增强ct', '病理报告', '基因检测', 'kras', 'braf', '微卫星', 'msi'], weight: 1 },
      ],
      'medication': [
        { keywords: ['药', '吃', '剂量', '医保', '报销', '副作用', '甲类', '乙类', '药品目录', '集采', '仿制药', '原研药', '靶向药', '免疫药', 'pd-1', 'pd-l1', '贝伐', '西妥昔', '雷莫西', '阿帕替尼', '安罗替尼', '仑伐', '索拉非尼', '瑞戈非尼', '喹替尼', '曲氟尿苷', '替吉奥', '卡培他滨', '奥沙利铂', '伊立替康', '氟尿嘧啶', '紫杉醇', '多西他赛', '白蛋白紫杉醇', '顺铂', '奈达铂', '洛铂'], weight: 2 },
        { keywords: ['怎么吃', '饭前', '饭后', '空腹', '一天几次', '一次几片', '漏服', '停药', '换药', '相互作用', '肝肾功能', '血常规', '白细胞低', '血小板低', '贫血', '恶心呕吐', '腹泻', '手足综合征', '皮疹', '口腔溃疡', '脱发'], weight: 1 },
      ],
      'chronic-disease': [
        { keywords: ['高血压', '糖尿病', '长期', '管理', '控制', '复查', '血糖', '血压', '血脂', '尿酸', '慢性', '随访', '监测', '生活方式', '饮食控制', '运动', '戒烟', '限酒', '体重管理', ' bmi', '腰围'], weight: 2 },
        { keywords: ['并发症', '靶器官', '眼底', '肾功能', '心电图', '颈动脉', '足部', '神经病变', '肾病', '视网膜病变', '心血管', '脑血管'], weight: 1 },
      ],
      'medical-guidance': [
        { keywords: ['医保', '报销', '费用', '特药', '双通道', '门特', '门规', '异地就医', '临床试验', '大病保险', '价格', '多少钱', '花费', '开药', '药占比', '医院没有', '买不到', '怎么开', '哪里买', '哪里能买', '政策', '新农合', '城乡居民', '挂号', '预约', '北肿', '北京肿瘤', '就诊', '转诊', '床位', '进口', '国产', '集采', '买药', '取药', '药房', '药店', '安宁疗护', '临终关怀', 'hospice', '病房', '社区服务', '社区卫生服务中心', '护理院', '养老院', '康复', '护理', '照护', 'hospice care', 'palliative', '备案', ' referral', '转院', '异地', '外地', '跨省', '保险', '社保', '商业保险', '理赔', '救助', '基金', '援助', '免费', '补贴', '减免', '哪里可以', '怎么申请', '流程', '手续', '材料', '证明'], weight: 2 },
        { keywords: ['住院', '门诊', '急诊', '手术费', '检查费', '药费', '自付', '自费', '起付线', '封顶线', '报销比例', '大病', '二次报销', '医疗救助', '慈善援助', '患者援助', '赠药', '免费用药'], weight: 1 },
      ],
    };
    ...
    const stageDescriptions: Record<string, string> = {
      ...
      'treatment-plan': '根据您的问题内容，将为您提供治疗相关的专业指导',
      'medication': '根据您的问题内容，将为您提供用药和医保相关的专业指导',
      'chronic-disease': '根据您的问题内容，将为您提供慢病管理和日常监测指导',
      'medical-guidance': '根据您的问题内容，将为您提供就医流程和医保政策指导',
    };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50 dark:from-slate-900 dark:via-gray-800 dark:to-slate-900">
      ...
      <main className="container mx-auto px-2 md:px-4 py-2 md:py-4 max-w-6xl">
        ...
        {/* Stage Navigation - 横向单行紧凑布局（所有模式统一） */}
                  ...
                  return (
                    <React.Fragment key={stage.id}>
                      <button
                        ...
                        className={`flex items-center gap-2 rounded-lg border-2 px-2.5 py-1.5 transition-all shrink-0 ${
                          isCurrent 
                      ...
                      >
                        ...
                        <div className="text-left">
                          <div className={`text-xs font-semibold leading-tight ${isCurrent ? 'text-blue-700 dark:text-blue-400' : isCompleted ? 'text-green-700 dark:text-green-400' : 'text-gray-700 dark:text-gray-300'}`}>
                            {stage.title}
                          </div>
                          <div className={`text-[10px] leading-tight ${isCurrent ? 'text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400'}`}>
                            {stage.description}
                          </div>
                        </div>
                      </button>
            ...
            <Card className="border-gray-200 dark:border-gray-700 bg-white dark:bg-slate-800 shadow-lg flex flex-col h-[calc(100vh-160px)] sm:h-[calc(100vh-140px)] md:h-[calc(100vh-80px)] lg:h-[calc(100vh-80px)]">
              ...
              <CardContent className="p-0 flex flex-col flex-1 min-h-0 overflow-hidden">
                        ...
                        <div key={message.id}>
                          ...
                          >
                            ...
                            >
                              ...
                              <div 
                                className="text-[13px] sm:text-sm leading-relaxed max-w-none break-words markdown-content"
                ...
                {(() => {
                  ...
                  const Icon = displayStage.icon;
                  return (
                    <div className="border-t border-gray-200 dark:border-gray-700 p-3 bg-gradient-to-r from-blue-50/50 to-purple-50/50 dark:from-blue-900/10 dark:to-purple-900/10">
                      <div className="flex items-start gap-2.5">
                        <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gradient-to-br ${displayStage.color} text-white`}>
                          <Icon className="h-4 w-4" />
                        </div>
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap gap-1">
                            {displayStage.features.map((feature, idx) => (
                              <span key={idx} className="inline-flex items-center text-[10px] bg-white/80 dark:bg-slate-800/80 text-gray-600 dark:text-gray-400 px-1.5 py-0.5 rounded">
                                <span className="text-blue-500 mr-0.5">•</span>
                                {feature}
                              </span>
                            ))}
